(function($) {
  ...
  $(function() {
    ...
    test("function toHtml", function() {
      var o = {type: "password", name: "repeat_password"};
      equal(toHtml(o),  'type="password" name="repeat_password"', "toHtml");
    });
    ...
    test("parse date", function() {
      var date = parseDate("2012-03-04", "y-M-d");
      var expected = new Date(2012, 2, 4);
      ok(dateEqual(date, expected), "date equal expected");
    });
    
    test("parse date and time", function() {
      var date = parseDate("08/09/2013 04:06:05", "d/M/y h:m:s");
      var expected = new Date(2013, 8, 8, 4, 6, 5);
      ok(dateEqual(date, expected), "date equal expected");
    });

    test("parse date invalid", function() {
      var date = parseDate("08/13/2013 04:06:05", "d/M/y h:m:s");
      equal(date, null, "date is null");
    });

    test("date to string format", function() {
      var date = parseDate("08/11/2013 04:06:05", "d/M/y h:m:s");
      equal(formatDate(date, "d/M/y h:m:s"), "08/11/2013 04:06:05", "date formated");
    });
